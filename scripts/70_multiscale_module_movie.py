# %% [markdown]
# # 70 · Multi-scale network analysis: module-distribution movie
#
# This script turns the static module maps from scripts 50/60 into a movie of
# the spatial coarse-graining sweep requested for the course:
#
# ```text
...
# holds each scale for three seconds; there are no interpolated transition frames.
...
# ## Settings
#
# Defaults: Mouse 1 sleep recording, its awake window, all active neurons, and
# Q-max clustering over 100 Louvain runs. Set ``DATASET = "anesthesia"`` to use
...
RECORDING = "mouse01_sleep"  # v2 filename for the requested Mouse 1 sleep data
STATE = "awake"  # sleep: "awake"/"nrem"; anesthesia: "awake"/"anesthesia"
...
MAX_NEURONS = None  # None uses every active neuron; set 800 for a fast preview
WINDOW_FRAMES = {"sleep": 1500, "ane": 2900}
DEFAULT_RECORDINGS = {"sleep": "mouse01_sleep", "anesthesia": "mouse05_ane"}
...
# its v2 recording file. ``RECORDING = None`` remains available to select the
...
#
# Each of the seven estimated partitions is held for three seconds, followed by
...
output = (
    ...
    / f"70_multiscale_modules_{recording_name}_{state}_{clustering_method}.mp4"
)
