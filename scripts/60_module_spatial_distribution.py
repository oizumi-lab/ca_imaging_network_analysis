# %% [markdown]
# # 60 · Spatial distribution of modules: single-cell (intermixed) vs mesoscale (localized)
#
...
K = 0.05
GAMMA = 1.0
N_RUNS = 20            # Louvain iterations for the distance-curve networks; take max-Q (paper: 200)
N_WINDOWS = 1          # windows per state for the distance curves
MAX_NEURONS = 2500     # subsample for the distance CURVES only (speed); None = all (like the paper)
...
N_RUNS_MAP = 30        # Louvain runs for the single-cell example maps (all neurons; slower)
...
fig.tight_layout()
fig.savefig(FIG_DIR / "60_module_maps.png", dpi=140, bbox_inches="tight")
plt.show()
...
fig.tight_layout()
fig.savefig(FIG_DIR / "60_same_module_vs_distance.png", dpi=140, bbox_inches="tight")
plt.show()
